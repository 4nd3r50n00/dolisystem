...
if ($action == "importSignature") {
	...
	if ($issignatureok) {
		$device_info = GETPOST('device_info', 'none');
		$audit_log = DOL_DATA_ROOT . '/signatures_audit.log';
		$audit_line = dol_print_date(dol_now(), "%Y-%m-%d %H:%M:%S") . " | IP: " . getUserRemoteIP() . " | Ref: " . $ref . " | Mode: " . $mode . " | Signer: " . $online_sign_name . " | Device: " . $device_info . "\n";
		@file_put_contents($audit_log, $audit_line, FILE_APPEND);
							...
							for ($i = 1; $i < ($pagecount + 1); $i++) {
								try {
									...
									if (empty($propalsignonspecificpage)) {
										...
										// the page x in PAGESIGN=x and Y position in YSIGN=x into $propalsignonspecificpage
										...
										$regy = array();
										if (preg_match('/YSIGN=([\d.]+)/', $keywords, $regy)) {
											$propalyforimgstart = (float) $regy[1];
										} else {
											$propalyforimgstart = null;
										}
									}
									...
									if (getDolGlobalString("PROPAL_SIGNATURE_ON_ALL_PAGES") || $propalsignonspecificpage == $i) {
										...
										if (getDolGlobalString("PROPAL_SIGNATURE_XFORIMGSTART")) {
											...
											$param['xforimgstart'] = (empty($s['w']) ? 49 : ($s['w'] - 112) / 2);
										}
										if (getDolGlobalString("PROPAL_SIGNATURE_YFORIMGSTART")) {
											$param['yforimgstart'] = getDolGlobalString("PROPAL_SIGNATURE_YFORIMGSTART");
										} elseif (!empty($propalyforimgstart)) {
											$param['yforimgstart'] = $propalyforimgstart; // From YSIGN= keyword in PDF
										} else {
					...
					if (dol_is_file($sourcefile)) {
						...
						if (empty($reshook)) {
							...
							if (!getDolGlobalString("FICHINTER_SIGNATURE_ON_ALL_PAGES")) {
								...
								$param['xforimgstart'] = (empty($s['w']) ? 65 : round($s['w'] / 2) - 40);
									$param['yforimgstart'] = (empty($s['h']) ? 242 : $s['h'] - 55);
								$param['wforimg'] = 80;
...
function dolPrintSignatureImage(TCPDF $pdf, $langs, $params)
{
	...
	// Draw the signature image first (rubrica), positioned at yforimgstart
	$pdf->Image($params['pathtoimage'], $xforimgstart, $yforimgstart, $wforimg, round($wforimg / 4));

	// Place audit text below the image, still inside the box
	$imgheight = round($wforimg / 4); // e.g. 112/4 = 28mm
	$pdf->SetXY($xforimgstart, $yforimgstart + $imgheight + 1);
	$pdf->SetFont($default_font, '', 7);
	$pdf->SetTextColor(80, 80, 80);
	// Build signature text with line breaks: date / Name / CPF-CNPJ
	$signline1 = $langs->transnoentities("Signature") . ': ' . dol_print_date(dol_now(), "day", false, $langs, true);
	$signline2 = $params['online_sign_name'];
	// Break CPF/CNPJ onto its own line
	$signline2 = str_replace(array(' (CPF', ' (CNPJ'), array("\n(CPF", "\n(CNPJ"), $signline2);
	$signtext = $signline1 . "\n" . $signline2;
	$pdf->MultiCell($wforimg, 3, $signtext, 0, 'C');
