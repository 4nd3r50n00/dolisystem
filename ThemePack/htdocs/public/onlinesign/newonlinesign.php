...
if ($source == 'proposal') {
	...
	// Creditor
	print '<tr class="CTableRow2"><td class="CTableRow2">'.$langs->transnoentitiesnoconv("Creditor");
	...
	// Debitor
	print '<tr class="CTableRow2"><td class="CTableRow2">'.$langs->trans("Customer");
	print '</td><td class="CTableRow2">';
	print img_picto('', 'company', 'class="pictofixedwidth"');
	...
	// Object
	$text = '<b>'.$langs->transnoentitiesnoconv("SignatureProposalRef", $object->ref).'</b>';
	...
	// Proposer
	print '<tr class="CTableRow2"><td class="CTableRow2">'.$langs->transnoentitiesnoconv("Proposer");
	print '</td><td class="CTableRow2">';
	print img_picto('', 'company', 'class="pictofixedwidth"');
	print '<b>'.$creditor.'</b>';
	print '<input type="hidden" name="creditor" value="'.$creditor.'">';
	...
	// Object
	$text = '<b>'.$langs->transnoentitiesnoconv("SignatureContractRef", $object->ref).'</b>';
	print '<tr class="CTableRow2"><td class="CTableRow2">'.$langs->transnoentitiesnoconv("Designation");
	print '</td><td class="CTableRow2">'.$text;
	...
	// Object
	$text = '<b>'.$langs->transnoentitiesnoconv("SignatureFichinterRef", $object->ref).'</b>';
	print '<tr class="CTableRow2"><td class="CTableRow2">'.$langs->transnoentitiesnoconv("Designation");
	print '</td><td class="CTableRow2">'.$text;
...
if ($action == "dosign" && empty($cancel)) {
	...
	print '<input type="text" class="paddingleftonly marginleftonly paddingrightonly marginrightonly marginbottomonly borderbottom" id="name"  placeholder="'.$langs->trans("FullName").'" autofocus><br>';
	print '<input type="text" class="paddingleftonly marginleftonly paddingrightonly marginrightonly marginbottomonly borderbottom" id="cpf"  placeholder="CPF / CNPJ"><br>';
	print '<label style="font-size: 0.9em; margin: 10px 0; display: inline-block;"><input type="checkbox" id="legalcheckbox" style="vertical-align: middle; margin-right: 5px;"> Declaro sob as penas da lei que os dados informados são autênticos e concordo com a validade jurídica desta assinatura eletrônica (MP 2.200-2/2001).</label>';
	print '<div style="margin-top: 15px; margin-bottom: 5px; font-weight: bold; font-size: 0.95em;">✏️ Desenhe sua assinatura (rubrica) no quadro abaixo:</div>';
	...
	$(document).ready(function() {
	  ...
	  $("#cpf").on("input", function(){
          ...
          if (this.value.length === 14 || this.value.length === 18) {
              if (isValidCpfCnpj(this.value)) {
                  $(this).attr("style", "border: 2px solid #00E676 !important; border-radius: 4px !important; box-shadow: 0 0 12px rgba(0, 230, 118, 0.6) !important;");
                  $(this).data("valid", "true");
              } else {
                  $(this).attr("style", "border: 2px solid #FF3D00 !important; border-radius: 4px !important; box-shadow: 0 0 12px rgba(255, 61, 0, 0.6) !important;");
	  ...
	  function checkFormValidity() {
		var hasSig = $("#signature").jSignature("getData", "native").length > 0;
		var hasName = $("#name").val().trim().length > 2;
		var hasCpf = $("#cpf").data("valid") === "true";
		var isChecked = $("#legalcheckbox").is(":checked");
		if (hasSig) { $("#clearsignature").css("display",""); }
		if (hasSig && hasName && hasCpf && isChecked) {
	  ...
	  $("#signature").on("change", checkFormValidity);
	  $("#name, #legalcheckbox").on("input change", checkFormValidity);
	  ...
	  $("#signbutton").on("click",function(){
		...
		var fullNameCpf = name + " (CPF: " + cpf + ")";
		
		var deviceInfo = "User-Agent: " + navigator.userAgent + " | OS: " + navigator.platform + " | Res: " + window.innerWidth + "x" + window.innerHeight;
	...
	if ($source == 'proposal') {
		if ($object->status == $object::STATUS_SIGNED) {
			print '<br>';
			if ($message == 'signed') {
				print '<div class="center ok">'.img_picto('', 'check', 'class="paddingright"', 0, 0, 0, '', 'size2x').' <strong>'.$langs->transnoentitiesnoconv("PropalSigned").'</strong></div>';
			} else {
				print '<div class="center ok">'.img_picto('', 'check', 'class="paddingright"', 0, 0, 0, '', 'size2x').' <strong>'.$langs->transnoentitiesnoconv("PropalAlreadySigned").'</strong></div>';
			}
		...
		if ($message == 'signed') {
			print '<div class="center ok">'.img_picto('', 'check', 'class="paddingright"', 0, 0, 0, '', 'size2x').' <strong>'.$langs->transnoentitiesnoconv("ContractSigned").'</strong></div>';
		} else {
		...
		if ($message == 'signed') {
			print '<div class="center ok">'.img_picto('', 'check', 'class="paddingright"', 0, 0, 0, '', 'size2x').' <strong>'.$langs->transnoentitiesnoconv("FichinterSigned").'</strong></div>';
		} else {
		...
		if ($message == 'signed' || $object->signed_status == Expedition::$SIGNED_STATUSES['STATUS_SIGNED_SENDER']) {
			print '<div class="center ok">'.img_picto('', 'check', 'class="paddingright"', 0, 0, 0, '', 'size2x').' <strong>'.$langs->transnoentitiesnoconv("ExpeditionSigned").'</strong></div>';
		} else {
