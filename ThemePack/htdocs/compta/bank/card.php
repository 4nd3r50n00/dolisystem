...
if ($action == 'create') {
	...
	if ((GETPOSTINT("id") || GETPOST("ref")) && $action != 'edit') {
		...
		if ($object->type == Account::TYPE_SAVINGS || $object->type == Account::TYPE_CURRENT) {
			...
			if (!empty($object->chave_pix)) {
				$label_tipo = $langs->trans("TipoChavePix".$object->tipo_chave_pix);
				print '<tr><td>'.$langs->trans("ChavePix").'</td>';
				print '<td>'.$object->chave_pix.' ('.$label_tipo.')</td>';
				print '</tr>';
			}
	...
	if (GETPOSTINT('id') && $action == 'edit' && $user->hasRight('banque', 'configurer')) {
		...
		if ($type == Account::TYPE_SAVINGS || $type == Account::TYPE_CURRENT) {
			...
			print '<tr><td>'.$langs->trans("TipoChavePix").'</td>';
			$tipo_chave_pix = array(
				'CPF' => $langs->trans("TipoChavePixCPF"),
				'CNPJ' => $langs->trans("TipoChavePixCNPJ"),
				'EMAIL' => $langs->trans("TipoChavePixEMAIL"),
				'TELEFONE' => $langs->trans("TipoChavePixTELEFONE"),
				'ALEATORIA' => $langs->trans("TipoChavePixALEATORIA")
			);
			print '<td>'.$form->selectarray('tipo_chave_pix', $tipo_chave_pix, GETPOSTISSET('tipo_chave_pix') ? GETPOST('tipo_chave_pix') : $object->tipo_chave_pix, 1).'</td></tr>';

			print '<tr><td>'.$langs->trans("ChavePix").'</td>';
			print '<td><input class="minwidth200 maxwidth300" maxlength="100" type="text" name="chave_pix" value="'.(GETPOSTISSET('chave_pix') ? GETPOST('chave_pix', 'alphanohtml') : $object->chave_pix).'"></td></tr>';
