...
if ($result >= 0) {
	...
	if ($action == 'add_paiement') {
		...
		if (!empty($totalpayment)) {
			$amountFormatted = 'R$ ' . price($totalpayment, 0, $langs, 1, -1, -1, '');
    		$text = $langs->trans('ConfirmCustomerPayment', $amountFormatted);
		}
		if (!empty($multicurrency_totalpayment)) {
			$amountWithSymbol = $langs->transnoentitiesnoconv("CurrencySymbol".$conf->currency) . ' ' . $totalpayment;
    		$text = $langs->trans('ConfirmCustomerPayment', $amountWithSymbol);
		}
