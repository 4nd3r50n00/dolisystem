 ...
 */
class pdf_master_propal extends ModelePDFPropales
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "MasterPropal";
		$this->description = 'Modelo Master Propal - By Anderson';
		...
		if ($conf->propal->multidir_output[$conf->entity]) {
			...
			if (file_exists($dir)) {
				...
				if (!getDolGlobalString('PROPAL_DISABLE_SIGNATURE')) {
					...
					// Use the actual box Y stored by drawSignatureArea so onlineSign stamps rubrica exactly inside it
					$ysign = isset($this->lastSignatureBoxY) ? round($this->lastSignatureBoxY, 2) : round($posy + 9, 2);
	...
	protected function drawSignatureArea(&$pdf, $object, $posy, $outputlangs)
	{
		...
		// --- Dimensions matching onlineSign.php rubrica placement ---
		$box_width  = 112; // matches $param['wforimg'] = 112 in onlineSign.php
		$box_height = 40;  // tall enough for rubrica (28mm) + audit text below
		$posx_sig   = ($this->page_largeur - $box_width) / 2; // = (210 - 112) / 2 = 49mm
		...
		$pdf->MultiCell($box_width, 5, "ASSINATURA DO CLIENTE", 0, 'C');

		// Signature box — Y must match yforimgstart in onlineSign.php
		$box_y = $label_y + 2; // a few mm below the label
		$pdf->SetDrawColor(150, 150, 150);
		$pdf->RoundedRect($posx_sig, $box_y, $box_width, $box_height, $this->corner_radius, '1234', 'D');

		// Return the Y value of the box so YSIGN can be set correctly in write_file
		$this->lastSignatureBoxY = $box_y;

		return $box_y + $box_height + 4;
	}
