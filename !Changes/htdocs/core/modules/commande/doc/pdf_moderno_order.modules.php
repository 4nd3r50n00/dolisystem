 ...
 */
class pdf_moderno_order extends ModelePDFCommandes
{
	/**
	...
	public function __construct(DoliDB $db)
	{
		...
		$this->db = $db;
		$this->name = "moderno_order";
		$this->description = $langs->trans('DocModelModernoOrderDescription');
			...
			if (file_exists($dir)) {
				...
				for ($i = 0; $i < $nblines; $i++) {
					...
					if ($this->getColumnStatus('desc')) {
						if ($object->lines[$i]->special_code != SUBTOTALS_SPECIAL_CODE) {
							// Force REFP uppercase
							if (!empty($object->lines[$i]->ref)) {
								$object->lines[$i]->ref = dol_strtoupper($object->lines[$i]->ref);
							}
					...
					// Add line
					if ($i < ($nblines - 1) && $afterPosData['y'] < $this->page_hauteur - $heightforfooter - 5) {
	...
	protected function drawInfoTable(&$pdf, $object, $posy, $outputlangs)
	{
		...
		if ($object->cond_reglement_code || $object->cond_reglement) {
			...
			$pdf->MultiCell($posxval - $this->marge_gauche - 1, 4, $titre, 0, 'L');
			...
			$pdf->MultiCell($this->page_largeur - $this->marge_droite - $posxval, 4, $lib_condition_paiement, 0, 'L');
		...
		*/
		...
		if (!empty($object->delivery_date)) {
			...
			$pdf->SetFont('', '', $default_font_size);
			$pdf->SetXY($posxval, $posy);
			$dlp = dol_print_date($object->delivery_date, "day", false, $outputlangs, true);
			$pdf->MultiCell($this->page_largeur - $this->marge_droite - $posxval, 4, $dlp, 0, 'L');
	...
	protected function drawTotalTable(&$pdf, $object, $deja_regle, $posy, $outputlangs, $outputlangsbis = null)
	{
		...
		$pdf->SetFont('', '', $default_font_size + 1);
        $total_ht_label = $outputlangs->transnoentities("TotalHT").(is_object($outputlangsbis) ? ' / '.$outputlangsbis->transnoentities("TotalHT") : '');
        if ($currency == 'BRL') $total_ht_label .= " (R$)";
		$pdf->MultiCell($col2x - $col1x, $tab2_hl, $total_ht_label, 0, 'L', true);
		$total_ht = ((isModEnabled("multicurrency") && $object->multicurrency_tx != 1) ? $object->multicurrency_total_ht : $object->total_ht);
		...
		if (!getDolGlobalString('MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT')) {
			...
			if (getDolGlobalString('MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_IFNULL') && $tvaisnull) {
				...
				// Draw a single background for the total row to ensure perfect alignment
				$pdf->Rect($col1x, $tab2_top + $tab2_hl * $index, ($col2x - $col1x) + $largcol2, $tab2_hl, 'F');
				
				$total_ttc_label = $outputlangs->transnoentities("TotalTTC").(is_object($outputlangsbis) ? ' / '.$outputlangsbis->transcountrynoentities("TotalTTC", $mysoc->country_code) : '');
				if ($currency == 'BRL') $total_ttc_label .= " (R$)";
				$pdf->MultiCell($col2x - $col1x, $tab2_hl, $total_ttc_label, 0, 'L', false);

				$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
				if (!isModEnabled("multicurrency") || $object->multicurrency_tx == 1 || getDolGlobalInt('MULTICURRENCY_SHOW_ALSO_MAIN_CURRENCY_ON_PDF') == 0) {
					$pdf->MultiCell($largcol2, $tab2_hl, price($total_ttc, 0, $outputlangs), 0, 'R', false);
				} else {
					$pdf->MultiCell($largcol2, $tab2_hl, price($total_ttc, 0, $outputlangs), 0, 'R', false);
	...
	protected function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0, $currency = '', $outputlangsbis = null)
	{
		...
		if (empty($hidetop)) {
			$currency_label = $outputlangs->transnoentitiesnoconv("Currency".$currency);
			if ($currency == 'BRL') {
				$currency_label = "Brasil Real (R$)";
			}
			$titre = $outputlangs->transnoentities("AmountInCurrency", $currency_label);
			if (getDolGlobalInt('PDF_USE_ALSO_LANGUAGE_CODE') && is_object($outputlangsbis)) {
				$titre .= ' - '.$outputlangsbis->transnoentities("AmountInCurrency", $outputlangs->transnoentitiesnoconv("Currency".$currency));
			}
			...
			$pdf->RoundedRect($this->marge_gauche, $tab_top, $this->page_largeur - $this->marge_droite - $this->marge_gauche, $this->tabTitleHeight, $this->corner_radius, '1111', 'F', array(), $this->primary_color);
		}
		...
		// Set white text and bold for titles
		$pdf->SetTextColor(255, 255, 255);
		$pdf->SetFont('', 'B', $default_font_size - 1);

		// Transform titles to uppercase temporarily
		$old_titles = array();
		foreach ($this->cols as $key => $val) {
			if (isset($val['title']['label'])) {
				$old_titles[$key] = $val['title']['label'];
				$this->cols[$key]['title']['label'] = dol_strtoupper($val['title']['label']);
			} elseif (isset($val['title']['textkey'])) {
				$old_titles[$key] = $val['title']['textkey'];
				$this->cols[$key]['title']['label'] = dol_strtoupper($outputlangs->transnoentities($val['title']['textkey']));
			}
		}

		// Header vertical lines: white and dotted
		$pdf->SetDrawColor(255, 255, 255);
		$pdf->SetLineStyle(array('dash' => '1,1', 'color' => array(255, 255, 255)));
		$this->pdfTabTitles($pdf, $tab_top, $this->tabTitleHeight, $outputlangs, $hidetop);

		// Body vertical lines: black and dotted
		$pdf->SetDrawColor(0, 0, 0);
		$pdf->SetLineStyle(array('dash' => '1,1', 'color' => array(0, 0, 0)));
		$this->_drawVerticalLines($pdf, $tab_top + $this->tabTitleHeight, $tab_height - $this->tabTitleHeight);

		// Restore titles
		foreach ($old_titles as $key => $val) {
			if (isset($this->cols[$key]['title']['textkey']) && $this->cols[$key]['title']['textkey'] == $val) {
				unset($this->cols[$key]['title']['label']);
			} else {
				$this->cols[$key]['title']['label'] = $val;
			}
		}
	 ...
	 * @return void
	 */
	protected function _drawVerticalLines(&$pdf, $top, $height)
	{
		foreach ($this->cols as $key => $val) {
			if (!empty($val['status']) && !empty($val['border-left'])) {
				$x = $this->getColumnContentXStart($key);
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null, $titlekey = "PdfOrderTitle")
	{
		...
		$text = dol_strtoupper($outputlangs->transnoentities("Order"))." ".$object->ref;
		...
		// 5. Reference Customer (Bottom Right of blue box)
		if ($object->ref_client) {
			$posy_ref = $this->marge_haute + 30;
			$pdf->SetXY($posx, $posy_ref);
		...
		$pdf->SetTextColor(0, 0, 0);
		return array('top_shift' => 0, 'shipp_shift' => 0);
	}
